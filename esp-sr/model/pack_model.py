...
def read_data(filename):
    ...
    with open(filename, "rb") as f:
        data = f.read()
    print(f"文件 {os.path.basename(filename)} 读取完成，大小: {len(data)} 字节")
    return data
...
def pack_models(model_path, out_file="srmodels.bin"):
    ...
    """
    print(f"开始打包模型，模型路径: {model_path}")
    ...
    for root, dirs, _ in os.walk(model_path):
        ...
        for model_name in dirs:
            ...
            for _, _, files in os.walk(model_dir):
                print(f"模型 {model_name} 包含 {len(files)} 个文件")
    ...
    print(f"总共找到 {model_num} 个模型，{file_num} 个文件")
    
    header_len = 4 + model_num*(32+4) + file_num*(32+4+4) 
    print(f"计算头部长度: {header_len} 字节")
    ...
    for key in models:
        ...
        for file_name in models[key]:
            ...
            if data_bin == None:
                ...
                print(f"    文件起始位置: {header_len}, 大小: {len(models[key][file_name])} 字节")
            else:
                data_start = header_len+len(data_bin)
                model_bin += struct.pack('I', data_start)
                print(f"    文件起始位置: {data_start}, 大小: {len(models[key][file_name])} 字节")
                data_bin += models[key][file_name]
                model_bin += struct.pack('I', len(models[key][file_name]))
        
        out_bin += model_bin
    
    assert len(out_bin) == header_len, f"头部长度不匹配: 实际={len(out_bin)}, 预期={header_len}"
    print(f"头部数据大小: {len(out_bin)} 字节")
    
    if data_bin != None:
        out_bin += data_bin
        print(f"数据部分大小: {len(data_bin)} 字节")
    
    print(f"总文件大小: {len(out_bin)} 字节")

    out_file = os.path.join(model_path, out_file)
    print(f"正在写入输出文件: {out_file}")
    with open(out_file, "wb") as f:
        f.write(out_bin)
    print(f"文件写入完成: {out_file}")
